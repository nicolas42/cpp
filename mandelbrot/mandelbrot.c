...
typedef struct mandelbrot_t { double x; double y; double zoom; int width; int height; int max_iterations; int padding; } mandelbrot_t;
...
void draw_mandelbrot(mandelbrot_t arg)
{
    ...
    for (int y = 0; y < image_height; y += 1){
        for (int x = 0; x < image_width; x += 1){
            ...
            {
                int rgb[3];
                HSVtoRGB((int)((i % 255) / 255.0 * 360.0), 1, 0.5, rgb);
...
int main(void)
{
    ...
    draw_mandelbrot((mandelbrot_t){ .x = -0.6999687500000003, .y = -0.2901249999999999, .zoom = 1024, .width = 1000, .height = 1000, .max_iterations = 1000 });
